...
var label_tween_duration_y: float = 1
...
func start_moving():
	if !go_up_tween:
		go_up_tween = create_tween().set_loops().set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
...
func stop_moving():
	...
	go_down_tween = create_tween().set_loops().set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
